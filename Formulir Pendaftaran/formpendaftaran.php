<!DOCTYPE html>
<html>
<head>
	<title></title>
	<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">
	<style>
		.warning {color:#FF0000;}
	</style>
</head>
<body>
<?php
// Deklarasi Variabel Error
$error_jenis_pendaftaran=""; $error_tanggal_masuk_sekolah=""; $error_nis=""; $error_nomer_peserta_ujian=""; $error_pernah_paud="";
$error_pernah_tk=""; $error_skhun=""; $error_ijazah=""; $error_hobi=""; $error_citacita=""; $error_nama=""; $error_jenis_kelamin="";
$error_nisn=""; $error_nik=""; $error_tempat_lahir=""; $error_tanggal_lahir=""; $error_agama=""; $error_berkebutuhan_khusus="";
$error_alamat=""; $error_rt=""; $error_rw=""; $error_dusun=""; $error_desa=""; $error_kecamatan=""; $error_kabupaten="";
$error_kode_pos=""; $error_tempat_tinggal=""; $error_transportasi=""; $error_jarak=""; $error_hp=""; $error_telp="";
$error_email=""; $error_nama_ibu=""; $error_pekerjaan_ibu=""; $error_pendidikan_ibu=""; $error_no_ibu="";
$error_nama_bapak=""; $error_pekerjaan_bapak=""; $error_pendidikan_bapak=""; $error_no_bapak="";
$error_nama_wali=""; $error_pekerjaan_wali=""; $error_pendidikan_wali=""; $error_no_wali="";
$error_penerima_kps=""; $error_no_kps=""; $error_kewarganegaraan="";
$error_nama_negara=""; $error_alasan="";

//Deklarasi Variabel 
$jenis_pendaftaran=""; $tanggal_masuk_sekolah=""; $nis=""; $nomer_peserta_ujian=""; $pernah_paud=""; $pernah_tk=""; $skhun=""; $ijazah="";
$hobi=""; $citacita=""; $nama=""; $jenis_kelamin=""; $nisn=""; $nik=""; $tempat_lahir=""; $tanggal_lahir=""; $agama="";
$berkebutuhan_khusus=""; $alamat=""; $rt=""; $rw=""; $dusun=""; $desa=""; $kecamatan=""; $kabupaten=""; $kode_pos="";
$tempat_tinggal=""; $transportasi=""; $jarak=""; $hp=""; $telp=""; $email=""; $nama_ibu=""; $pekerjaan_ibu="";
$pendidikan_ibu=""; $no_ibu=""; $nama_bapak=""; $pekerjaan_bapak=""; $pendidikan_bapak=""; $no_bapak="";
$nama_wali=""; $pekerjaan_wali=""; $pendidikan_wali=""; $no_wali=""; $penerima_kps=""; $no_kps="";
$kewarganegaraan=""; $nama_negara=""; $alasan="";

$pesan_sukses="";

// Jika method POST dijalankan
if ($_SERVER["REQUEST_METHOD"] == "POST"){
	//Jenis Pendaftaran
	if(!isset($_POST["jenis_pendaftaran"])){
		$error_jenis_pendaftaran="Jenis Pendaftaran tidak boleh kosong";
	}
	else{
		$jenis_pendaftaran=cek_input($_POST["jenis_pendaftaran"]);
		if($jenis_pendaftaran == ""){
			$error_jenis_pendaftaran="Jenis Pendaftaran Harus Dipilih";	
		}
		else{
			$error_jenis_pendaftaran="";
		}
	}

	// Tanggal Daftar
	if(empty($_POST["tanggal_masuk_sekolah"])){
		$error_tanggal_masuk_sekolah="Tanggal Daftar tidak boleh kosong";
	}
	else{
		$tanggal_masuk_sekolah=cek_input($_POST["tanggal_masuk_sekolah"]);
		$tanggal_masuk_sekolah=date('Y-m-d', strtotime($tanggal_masuk_sekolah));
	}

	// NIS
	if(empty($_POST["nis"])){
		$error_nis="NIS tidak boleh kosong";
	}
	else{
		$nis=cek_input($_POST["nis"]);
		if(!is_numeric($nis)){
			$error_nis="NIS hanya boleh berisi angka";
		}
		elseif (strlen($nis) != 10) {
			$error_nis="Panjang NIS harus 10 digit";
		}
	}

	// Nomer Peserta Ujian
	if(empty($_POST["nomer_peserta_ujian"])){
		$error_nomer_peserta_ujian="Nomer Peserta Ujian tidak boleh kosong";
	}
	else{
		$nomer_peserta_ujian=cek_input($_POST["nomer_peserta_ujian"]);
		if(!is_numeric($nomer_peserta_ujian)){
			$error_nomer_peserta_ujian="Nomer Peserta Ujian hanya boleh berisi angka";
		}
		elseif (strlen($nomer_peserta_ujian) != 8) {
			$error_nomer_peserta_ujian="Panjang Nomer Peserta Ujian harus 8 digit";
		}
	}

	// Pernah PAUD
	if(empty($_POST["pernah_paud"])){
		$error_pernah_paud="Pilihan tidak boleh kosong";
	}
	else{
		$pernah_paud=cek_input($_POST["pernah_paud"]);
	}

	// Pernah TK
	if(empty($_POST["pernah_tk"])){
		$error_pernah_tk="Pilihan tidak boleh kosong";
	}
	else{
		$pernah_tk=cek_input($_POST["pernah_tk"]);
	}

	// SKHUN
	if(empty($_POST["skhun"])){
		$error_skhun="Nomer Seri SKHUN tidak boleh kosong";
	}
	else{
		$skhun=cek_input($_POST["skhun"]);
		if(!is_numeric($skhun)){
			$error_skhun="Nomer Seri SKHUN hanya boleh berisi angka";
		}
		elseif (strlen($skhun) != 7) {
			$error_skhun="Panjang Nomer Seri SKHUN harus 7 digit";
		}
	}

	// Ijazah
	if(empty($_POST["ijazah"])){
		$error_ijazah="Nomer Seri Ijazah tidak boleh kosong";
	}
	else{
		$ijazah=cek_input($_POST["ijazah"]);
		if(!is_numeric($ijazah)){
			$error_ijazah="Nomer Seri Ijazah hanya boleh berisi angka";
		}
		elseif (strlen($ijazah) != 7) {
			$error_ijazah="Panjang Nomer Seri Ijazah harus 7 digit";
		}
	}

	// Hobi
	if(!isset($_POST["hobi"])){
		$error_hobi="Kolom Hobi Tidak Boleh Kosong";
	}
	else{
		$hobi=cek_input($_POST["hobi"]);
		if($hobi == ""){
			$error_hobi="Hobi Harus Dipilih";	
		}
		else{
			$error_hobi="";
		}
	}

	// Cita-cita
	if(!isset($_POST["citacita"])){
		$error_citacita="Kolom Cita-cita Tidak Boleh Kosong";
	}
	else{
		$citacita=cek_input($_POST["citacita"]);
		if($citacita == ""){
			$error_citacita="Cita-cita Harus Dipilih";	
		}
		else{
			$error_citacita="";
		}
	}

	// Nama Lengkap
	if(empty($_POST["nama"])){
		$error_nama="Nama tidak boleh kosong";
	}
	else{
		$nama=cek_input($_POST["nama"]);
		if(!preg_match("/^[a-zA-z ]*$/", $nama)){
			$error_nama="Inputan Hanya Boleh Huruf dan Spasi";
		}
	}

	// Jenis Kelamin
	if(empty($_POST["jenis_kelamin"])){
		$error_jenis_kelamin="Pilihan tidak boleh kosong";
	}
	else{
		$jenis_kelamin=cek_input($_POST["jenis_kelamin"]);
	}

	// NISN
	if(empty($_POST["nisn"])){
		$error_nisn="NISN tidak boleh kosong";
	}
	else{
		$nisn=cek_input($_POST["nisn"]);
		if(!is_numeric($nisn)){
			$error_nisn="NISN hanya boleh berisi angka";
		}
		elseif (strlen($nisn) != 8) {
			$error_nisn="Panjang NISN harus 8 digit";
		}
	}

	// NIK
	if(empty($_POST["nik"])){
		$error_nik="NIK tidak boleh kosong";
	}
	else{
		$nik=cek_input($_POST["nik"]);
		if(!is_numeric($nik)){
			$error_nik="NIK hanya boleh berisi angka";
		}
		elseif (strlen($nik) != 16) {
			$error_nik="Panjang NIK harus 16 digit";
		}
	}

	// Tempat Lahir
	if(empty($_POST["tempat_lahir"])){
		$error_tempat_lahir="Tempat lahir tidak boleh kosong";
	}
	else{
		$tempat_lahir=cek_input($_POST["tempat_lahir"]);
		if(!preg_match("/^[a-zA-z ]*$/", $tempat_lahir)){
			$error_tempat_lahir="Inputan Hanya Boleh Huruf dan Spasi";
		}
	}

	// Tanggal Lahir
	if(empty($_POST["tanggal_lahir"])){
		$error_tanggal_lahir="Tanggal Lahir tidak boleh kosong";
	}
	else{
		$tanggal_lahir=cek_input($_POST["tanggal_lahir"]);
		$tanggal_lahir=date('Y-m-d', strtotime($tanggal_lahir));
	}

	// Agama
	if(!isset($_POST["agama"])){
		$error_agama="Kolom Agama Tidak Boleh Kosong";
	}
	else{
		$agama=cek_input($_POST["agama"]);
		if($agama == ""){
			$error_agama="Agama Harus Dipilih";	
		}
		else{
			$error_agama="";
		}
	}

	// Berkebutuhan Khusus
	if(empty($_POST["berkebutuhan_khusus"])){
		$error_berkebutuhan_khusus="Berkebutuhan Khusus tidak boleh kosong";
	}
	else{
		$berkebutuhan_khusus=cek_input($_POST["berkebutuhan_khusus"]);
		if($berkebutuhan_khusus == "01"){
			$berkebutuhan_khusus="Tidak";	
		}
		elseif($berkebutuhan_khusus == "02"){
			$berkebutuhan_khusus="Netra (A)";	
		}
		elseif($berkebutuhan_khusus == "03"){
			$berkebutuhan_khusus="Rungu (B)";	
		}
		elseif($berkebutuhan_khusus == "04"){
			$berkebutuhan_khusus="Grahita Ringan (C)";	
		}
		elseif($berkebutuhan_khusus == "05"){
			$berkebutuhan_khusus="Grahita Sedang (C1)";	
		}
		elseif($berkebutuhan_khusus == "06"){
			$berkebutuhan_khusus="Daksa Ringan (D)";	
		}
		elseif($berkebutuhan_khusus == "07"){
			$berkebutuhan_khusus="Daksa Sedang (D1)";	
		}
		elseif($berkebutuhan_khusus == "08"){
			$berkebutuhan_khusus="Laras (E)";	
		}
		elseif($berkebutuhan_khusus == "09"){
			$berkebutuhan_khusus="Wicara (F)";	
		}
		elseif($berkebutuhan_khusus == "10"){
			$berkebutuhan_khusus="Tuna Ganda (B)";	
		}
		elseif($berkebutuhan_khusus == "11"){
			$berkebutuhan_khusus="Hiper Aktif (H)";	
		}
		elseif($berkebutuhan_khusus == "12"){
			$berkebutuhan_khusus="Cerdas Istimewa (i)";	
		}
		elseif($berkebutuhan_khusus == "13"){
			$berkebutuhan_khusus="Bakat Istimewa (J)";	
		}
		elseif($berkebutuhan_khusus == "14"){
			$berkebutuhan_khusus="Kesulitan Belajar (K)";	
		}
		elseif($berkebutuhan_khusus == "15"){
			$berkebutuhan_khusus="Narkoba (N)";	
		}
		elseif($berkebutuhan_khusus == "16"){
			$berkebutuhan_khusus="Indigo (O)";	
		}
		elseif($berkebutuhan_khusus == "17"){
			$berkebutuhan_khusus="Down Sindrome (P)";	
		}
		elseif($berkebutuhan_khusus == "18"){
			$berkebutuhan_khusus="Autis (Q)";	
		}
		else{
			$error_berkebutuhan_khusus="Masukkan pilihan berkebutuhan khusus dengan benar";
		}
	}

	// Alamat Jalan
	if(empty($_POST["alamat"])){
		$error_alamat="Alamat Jalan tidak boleh kosong";
	}
	else{
		$alamat=cek_input($_POST["alamat"]);
	}

	// RT
	if(empty($_POST["rt"])){
		$error_rt="RT tidak boleh kosong";
	}
	else{
		$rt=cek_input($_POST["rt"]);
		if(!is_numeric($rt)){
			$error_rt="RT hanya boleh berisi angka";
		}
	}

	// RW
	if(empty($_POST["rw"])){
		$error_rw="RW tidak boleh kosong";
	}
	else{
		$rw=cek_input($_POST["rw"]);
		if(!is_numeric($rw)){
			$error_rw="RW hanya boleh berisi angka";
		}
	}

	// Dusun
	if(empty($_POST["dusun"])){
		$error_dusun="Dusun tidak boleh kosong";
	}
	else{
		$dusun=cek_input($_POST["dusun"]);
		if(!preg_match("/^[a-zA-z ]*$/", $dusun)){
			$error_dusun="Inputan Hanya Boleh Huruf dan Spasi";
		}
	}	

	// Desa/Kelurahan
	if(empty($_POST["desa"])){
		$error_desa="Desa/Kelurahan tidak boleh kosong";
	}
	else{
		$desa=cek_input($_POST["desa"]);
		if(!preg_match("/^[a-zA-z ]*$/", $desa)){
			$error_desa="Inputan Hanya Boleh Huruf dan Spasi";
		}
	}

	// Kecamatan
	if(empty($_POST["kecamatan"])){
		$error_kecamatan="Kecamatan tidak boleh kosong";
	}
	else{
		$kecamatan=cek_input($_POST["kecamatan"]);
		if(!preg_match("/^[a-zA-z ]*$/", $kecamatan)){
			$error_kecamatan="Inputan Hanya Boleh Huruf dan Spasi";
		}
	}

	// Kabupaten
	if(empty($_POST["kabupaten"])){
		$error_kabupaten="Kabupaten tidak boleh kosong";
	}
	else{
		$kabupaten=cek_input($_POST["kabupaten"]);
		if(!preg_match("/^[a-zA-z ]*$/", $kabupaten)){
			$error_kabupaten="Inputan Hanya Boleh Huruf dan Spasi";
		}
	}

	// Kode Pos
	if(empty($_POST["kode_pos"])){
		$error_kode_pos="Kode Pos tidak boleh kosong";
	}
	else{
		$kode_pos=cek_input($_POST["kode_pos"]);
		if(!is_numeric($kode_pos)){
			$error_kode_pos="Kode Pos hanya boleh berisi angka";
		}
	}

	// Tempat Tinggal
	if(!isset($_POST["tempat_tinggal"])){
		$error_tempat_tinggal="Kolom ini Tidak Boleh Kosong";
	}
	else{
		$tempat_tinggal=cek_input($_POST["tempat_tinggal"]);
		if($tempat_tinggal == ""){
			$error_tempat_tinggal="Tempat Tinggal Harus Dipilih";	
		}
		else{
			$error_tempat_tinggal="";
		}
	}

	// Moda Transportasi
	if(empty($_POST["transportasi"])){
		$error_transportasi="Moda Transportasi tidak boleh kosong";
	}
	else{
		$transportasi=cek_input($_POST["transportasi"]);
		if($transportasi == "01"){
			$transportasi="Jalan Kaki";	
		}
		elseif($transportasi == "02"){
			$transportasi="Kendaraan Pribadi";	
		}
		elseif($transportasi == "03"){
			$transportasi="Kendaraan Umum/Angkot/Pete-pete";	
		}
		elseif($transportasi == "04"){
			$transportasi="Jemputan Sekolah";	
		}
		elseif($transportasi == "05"){
			$transportasi="Kereta Api";	
		}
		elseif($transportasi == "06"){
			$transportasi="Ojek";	
		}
		elseif($transportasi == "07"){
			$transportasi="Andong/Bendi/Sado/Dokar/Delman/Becak";	
		}
		elseif($transportasi == "08"){
			$transportasi="Perahu Penyeberangan/Rakit/Getek";	
		}
		elseif($transportasi == "09"){
			$transportasi="Lainnya";	
		}
		else{
			$error_transportasi="Masukkan pilihan kendaraan dengan benar";
		}
	}

	// Jarak ke Sekolah
	if(empty($_POST["jarak"])){
		$error_jarak="Isian tidak boleh kosong";
	}
	else{
		$jarak=cek_input($_POST["jarak"]);
		if(!is_numeric($jarak)){
			$error_jarak="Jarak hanya boleh angka";
		}
	}

	// No. HP
	if(empty($_POST["hp"])){
		$error_hp="No. HP tidak boleh kosong";
	}
	else{
		$hp=cek_input($_POST["hp"]);
		if(!is_numeric($hp)){
			$error_hp="Nomor HP hanya boleh angka";
		}
	}

	// Email
	if(empty($_POST["email"])){
		$error_email="Email tidak boleh kosong";
	}
	else{
		$email=cek_input($_POST["email"]);
		if(!filter_var($email, FILTER_VALIDATE_EMAIL)){
			$error_email="Format Email Invalid";
		}
	}

	// No. Telepon
	$telp=cek_input($_POST["telp"]);
	if(!is_numeric($telp)){
		$error_telp="Nomor Telepon hanya boleh angka";
	}

	// Nama Ibu
	if(empty($_POST["nama_ibu"])){
		$error_nama_ibu="Nama tidak boleh kosong";
	}
	else{
		$nama_ibu=cek_input($_POST["nama_ibu"]);
		if(!preg_match("/^[a-zA-z ]*$/", $nama_ibu)){
			$error_nama_ibu="Inputan Hanya Boleh Huruf dan Spasi";
		}
	}
	//Pekerjaan Ibu
	if(empty($_POST["pekerjaan_ibu"])){
		$error_pekerjaan_ibu="Pekerjaan Ibu tidak boleh kosong";
	}
	else{
		$pekerjaan_ibu=cek_input($_POST["pekerjaan_ibu"]);
	}
	//Pendidikan Bapak
	if(empty($_POST["pendidikan_ibu"])){
		$error_pendidikan_ibu="Pendidikan Ibu tidak boleh kosong";
	}
	else{
		$pendidikan_ibu=cek_input($_POST["pendidikan_ibu"]);
	}
	//Nomor Telepon Ibu
	if(empty($_POST["no_ibu"])){
		$error_no_ibu="Telp tidak boleh kosong";
	}
	else{
		$no_ibu=cek_input($_POST["no_ibu"]);
		if(!is_numeric($no_ibu)){
			$error_no_ibu="Nomor Telepon hanya boleh angka";
		}
	}

	// Nama Bapak
	if(empty($_POST["nama_bapak"])){
		$error_nama_bapak="Nama tidak boleh kosong";
	}
	else{
		$nama_bapak=cek_input($_POST["nama_bapak"]);
		if(!preg_match("/^[a-zA-z ]*$/", $nama_bapak)){
			$error_nama_bapak="Inputan Hanya Boleh Huruf dan Spasi";
		}
	}
	//Pekerjaan Bapak
	if(empty($_POST["pekerjaan_bapak"])){
		$error_pekerjaan_bapak="Pekerjaan Ibu tidak boleh kosong";
	}
	else{
		$pekerjaan_bapak=cek_input($_POST["pekerjaan_bapak"]);
	}
	//Pendidikan Bapak
	if(empty($_POST["pendidikan_bapak"])){
		$error_pendidikan_bapak="Pendidikan Ibu tidak boleh kosong";
	}
	else{
		$pendidikan_bapak=cek_input($_POST["pendidikan_bapak"]);
	}
	//Nomor Telepon Bapak
	if(empty($_POST["no_bapak"])){
		$error_no_bapak="Telp tidak boleh kosong";
	}
	else{
		$no_bapak=cek_input($_POST["no_bapak"]);
		if(!is_numeric($no_bapak)){
			$error_no_bapak="Nomor Telepon hanya boleh angka";
		}
	}

	// Nama Wali
	if(empty($_POST["nama_wali"])){
		$error_nama_wali="Nama tidak boleh kosong";
	}
	else{
		$nama_wali=cek_input($_POST["nama_wali"]);
		if(!preg_match("/^[a-zA-z ]*$/", $nama_wali)){
			$error_nama_wali="Inputan Hanya Boleh Huruf dan Spasi";
		}
	}
	//Pekerjaan Wali
	if(empty($_POST["pekerjaan_wali"])){
		$error_pekerjaan_wali="Pekerjaan Ibu tidak boleh kosong";
	}
	else{
		$pekerjaan_wali=cek_input($_POST["pekerjaan_wali"]);
	}
	//Pendidikan Wali
	if(empty($_POST["pendidikan_wali"])){
		$error_pendidikan_wali="Pendidikan Ibu tidak boleh kosong";
	}
	else{
		$pendidikan_wali=cek_input($_POST["pendidikan_wali"]);
	}
	//Nomor Telepon Wali
	if(empty($_POST["no_wali"])){
		$error_no_wali="Telp tidak boleh kosong";
	}
	else{
		$no_wali=cek_input($_POST["no_wali"]);
		if(!is_numeric($no_wali)){
			$error_no_wali="Nomor Telepon hanya boleh angka";
		}
	}

	// Penerima KPS/PKH/KIP
	if(empty($_POST["penerima_kps"])){
		$error_penerima_kps="Pilihan tidak boleh kosong";
	}
	else{
		$penerima_kps=cek_input($_POST["penerima_kps"]);
	}

	// Nomor KPS/KKS/PKH/KIP
	$no_kps=cek_input($_POST["no_kps"]);
	if(!is_numeric($no_kps)){
		$error_no_kps="Nomor KPS/KKS/PKH/KIP hanya boleh angka";
	}

	// Kewarganegaraan
	if(empty($_POST["kewarganegaraan"])){
		$error_kewarganegaraan="Pilihan tidak boleh kosong";
	}
	else{
		$kewarganegaraan=cek_input($_POST["kewarganegaraan"]);
	}	

	// Nama Negara
	if ($kewarganegaraan == "WNA") {
		if(empty($_POST["nama_negara"])){
			$error_nama_negara="Nama Negara tidak boleh kosong";
		}
		else{
			$nama_negara=cek_input($_POST["nama_negara"]);
			if(!preg_match("/^[a-zA-z ]*$/", $nama_negara)){
				$error_nama_negara="Inputan Hanya Boleh Huruf dan Spasi";
			}
		}
	}
	elseif ($kewarganegaraan == "WNI") {
		$nama_negara="Indonesia";
	}

	//Alasan mendaftar
	if (empty($_POST["alasan"])) {
				$error_alasan = "Textbox tidak boleh kosong"; 
			}
			else {
				$alasan = cek_input($_POST["alasan"]); 
			}

	// Jika tidak ada error maka data akan disimpan
	if ($error_jenis_pendaftaran =="" && $error_tanggal_masuk_sekolah=="" && $error_nis =="" && $error_nomer_peserta_ujian=="" && $error_pernah_paud=="" && $error_pernah_tk=="" && $error_skhun=="" && $error_ijazah=="" && $error_hobi=="" && $error_citacita=="" && $error_nama=="" && $error_jenis_kelamin=="" && $error_nisn=="" && $error_nik=="" && $error_tempat_lahir=="" && $error_tanggal_lahir=="" && $error_agama=="" && $error_berkebutuhan_khusus=="" && $error_alamat=="" && $error_rt=="" && $error_rw=="" && $error_dusun=="" && $error_desa=="" && $error_kecamatan=="" && $error_kabupaten=="" && $error_kode_pos=="" && $error_tempat_tinggal=="" && $error_transportasi=="" && $error_transportasi=="" && $error_hp=="" && $error_email=="" && $error_telp=="" && $error_nama_ibu=="" && $error_pekerjaan_ibu=="" && $error_pendidikan_ibu=="" && $error_no_ibu=="" && $error_nama_bapak=="" && $error_pekerjaan_bapak=="" && $error_pendidikan_bapak=="" && $error_no_bapak=="" && $error_nama_wali=="" && $error_pekerjaan_wali=="" && $error_pendidikan_wali=="" && $error_no_wali=="" && $error_penerima_kps=="" && $error_no_kps=="" && $error_kewarganegaraan=="" && $error_nama_negara=="" && $error_alasan=="") {
	
		include 'koneksi.php';

		// Memasukkan data ke tabel daftar
		$sql="INSERT INTO daftar VALUES ('$jenis_pendaftaran', '$tanggal_masuk_sekolah', '$nis', '$nomer_peserta_ujian', '$pernah_paud', '$pernah_tk', '$skhun', '$ijazah', '$hobi', '$citacita', '$nama', '$jenis_kelamin', '$nisn', '$nik', '$tempat_lahir', '$tanggal_lahir', '$agama', '$berkebutuhan_khusus', '$alamat', '$rt', '$rw', '$dusun', '$desa', '$kecamatan', '$kabupaten', '$kode_pos', '$tempat_tinggal', '$transportasi', '$jarak', '$hp', '$telp', '$email', '$nama_ibu', '$pekerjaan_ibu', '$pendidikan_ibu', '$no_ibu', '$nama_bapak', '$pekerjaan_bapak', '$pendidikan_bapak', '$no_bapak', '$nama_wali', '$pekerjaan_wali', '$pendidikan_wali', '$no_wali', '$penerima_kps', '$no_kps', '$kewarganegaraan', '$nama_negara', '$alasan')";
		mysqli_query($koneksi, $sql);

		$pesan_sukses="Berhasil simpan data";
	}
	else{
		$pesan_sukses="Gagal simpan data";
	}
}

//pengaturan karakter data inputan dari mulai spasi dan lainnya	
function cek_input($data){
	$data = trim($data);
	$data = stripslashes($data);
	$data = htmlspecialchars($data);
	return $data; //pengembalian nilai data inputan
}

?>

<!--Pengaturan Form Menggunakan Library Bootstrap-->
<div class="container">
	<div class="col-md-12">
		<div class="card">
			<h2 class="alert alert-primary text-center mt-2">
				FORMULIR PENDAFTARAN PESERTA DIDIK BARU
			</h2>
			<div class="card-body">
				<!-- Mengarahkan action ke halaman itu sendiri -->
				<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
					<!-- Jenis Pendaftaran -->
					<div class="form-group row">
						<label for="jenis_pendaftaran" class="col-sm-3 col-form-label">Jenis Pendaftaran</label>
						<div class="col-sm-9">
							<select name="jenis_pendaftaran" class="form-control <?php echo ($error_jenis_pendaftaran !="" ? "is_invalid" : ""); ?>" id="jenis_pendaftaran" placeholder="Pilih jenis pendaftaran" value="<?php echo $jenis_pendaftaran; ?>">
								<option value=""></option>
								<option value="Siswa Baru" <?php if (isset($jenis_pendaftaran) && $jenis_pendaftaran == "Siswa Baru") echo "selected";?>>Siswa Baru</option>
								<option value="Pindahan" <?php if (isset($jenis_pendaftaran) && $jenis_pendaftaran == "Pindahan") echo "selected";?>>Pindahan</option>
							</select><span class="warning"><?php echo $error_jenis_pendaftaran; ?></span>
						</div>
					</div> <br>

					<!-- Tanggal Daftar -->
					<div class="form-group row">
						<label for="tanggal_masuk_sekolah" class="col-sm-3 col-form-label">Tanggal Daftar</label>
						<div class="col-sm-9">
							<input type="date" name="tanggal_masuk_sekolah" class="form-control <?php echo ($error_tanggal_masuk_sekolah !="" ? "is_invalid" : ""); ?>" id="tanggal_masuk_sekolah" value="<?php echo $tanggal_masuk_sekolah; ?>"><span class="warning"><?php echo $error_tanggal_masuk_sekolah; ?></span>
						</div>
					</div><br>

					<!-- Pembatas Untuk Biodata Siswa -->
					<h4 class="alert alert-primary text-center mt-1">
							DATA PESERTA DIDIK
					</h4>	

					<!-- Nomor Induk Siswa -->
					<div class="form-group row">
						<label for="nis" class="col-sm-3 col-form-label">NIS</label>
						<div class="col-sm-9">
							<input type="text" name="nis" class="form-control <?php echo ($error_nis !="" ? "is_invalid" : ""); ?>" id="nis" placeholder="Masukkan NIS Anda" value="<?php echo $nis; ?>"><span class="warning"><?php echo $error_nis; ?></span>
						</div>
					</div> <br>

					<!-- Nomor Peserta Ujian -->
					<div class="form-group row">
						<label for="nomer_peserta_ujian" class="col-sm-3 col-form-label">Nomor Peserta Ujian</label>
						<div class="col-sm-9">
							<input type="text" name="nomer_peserta_ujian" class="form-control <?php echo ($error_nomer_peserta_ujian !="" ? "is_invalid" : ""); ?>" id="nomer_peserta_ujian" placeholder="Masukkan Nomor Peserta Ujian" value="<?php echo $nomer_peserta_ujian; ?>"><span class="warning"><?php echo $error_nomer_peserta_ujian; ?></span>
						</div>
					</div> <br>

					<!-- Radio Button PAUD -->
					<div class="form-group row">
						<label for="pernah_paud" class="col-sm-3 col-form-label">Apakah Pernah PAUD?</label>
						<div class="col-sm-9">
							<input type="radio" name="pernah_paud" value="Ya">Ya
							<input type="radio" name="pernah_paud" value="Tidak">Tidak
							<span class="warning"><?php echo $error_pernah_paud; ?></span>
						</div>
					</div> <br>

					<!-- Radio Button TK -->
					<div class="form-group row">
						<label for="pernah_tk" class="col-sm-3 col-form-label">Apakah Pernah TK?</label>
						<div class="col-sm-9">
							<input type="radio" name="pernah_tk" value="Ya">Ya
							<input type="radio" name="pernah_tk" value="Tidak">Tidak
							<span class="warning"><?php echo $error_pernah_tk; ?></span>
						</div>
					</div> <br>

					<!-- Nomor SKHUN -->
					<div class="form-group row">
						<label for="skhun" class="col-sm-3 col-form-label">Nomor SKHUN</label>
						<div class="col-sm-9">
							<input type="text" name="skhun" class="form-control <?php echo ($skhun !="" ? "is_invalid" : ""); ?>" id="skhun" placeholder="Masukkan No. Seri SKHUN Sebelumnya" value="<?php echo $skhun; ?>"><span class="warning"><?php echo $error_skhun; ?></span>
						</div>
					</div> <br>

					<!-- Nomor Ijazah -->
					<div class="form-group row">
						<label for="ijazah" class="col-sm-3 col-form-label">Nomor Ijazah</label>
						<div class="col-sm-9">
							<input type="text" name="ijazah" class="form-control <?php echo ($ijazah !="" ? "is_invalid" : ""); ?>" id="ijazah" placeholder="Masukkan No. Seri Ijazah Sebelumnya" value="<?php echo $ijazah; ?>"><span class="warning"><?php echo $error_ijazah; ?></span>
						</div>
					</div> <br>

					<!-- Nama Siswa -->
					<div class="form-group row">
						<label for="nama" class="col-sm-3 col-form-label">Nama Lengkap</label>
						<div class="col-sm-9">
							<input type="text" name="nama" class="form-control <?php echo ($error_nama !="" ? "is_invalid" : ""); ?>" id="nama" placeholder="Nama Lengkap Anda" value="<?php echo $nama; ?>"><span class="warning"><?php echo $error_nama; ?></span>
						</div>
					</div> <br>

					<!-- Gender Siswa -->
					<div class="form-group row">
						<label for="jenis_kelamin" class="col-sm-3 col-form-label">Jenis Kelamin</label>
						<div class="col-sm-9">
							<input type="radio" name="jenis_kelamin" value="Laki-laki">Laki-laki
							<input type="radio" name="jenis_kelamin" value="Perempuan">Perempuan
							<span class="warning"><?php echo $error_jenis_kelamin; ?></span>
						</div>
					</div> <br>

					<!-- NISN Siswa -->
					<div class="form-group row">
						<label for="nisn" class="col-sm-3 col-form-label">NISN</label>
						<div class="col-sm-9">
							<input type="text" name="nisn" class="form-control <?php echo ($error_nisn !="" ? "is_invalid" : ""); ?>" id="nisn" placeholder="Masukkan NISN Anda" value="<?php echo $nisn; ?>"><span class="warning"><?php echo $error_nisn; ?></span>
						</div>
					</div> <br>

					<!-- NIK Siswa -->
					<div class="form-group row">
						<label for="nik" class="col-sm-3 col-form-label">NIK</label>
						<div class="col-sm-9">
							<input type="text" name="nik" class="form-control <?php echo ($error_nik !="" ? "is_invalid" : ""); ?>" id="nik" placeholder="Masukkan NIK Anda" value="<?php echo $nik; ?>"><span class="warning"><?php echo $error_nik; ?></span>
						</div>
					</div> <br>

					<!-- Tempat Lahir Siswa -->
					<div class="form-group row">
						<label for="tempat_lahir" class="col-sm-3 col-form-label">Tempat Lahir</label>
						<div class="col-sm-9">
							<input type="text" name="tempat_lahir" class="form-control <?php echo ($error_tempat_lahir !="" ? "is_invalid" : ""); ?>" id="tempat_lahir" placeholder="Tempat Lahir" value="<?php echo $tempat_lahir; ?>"><span class="warning"><?php echo $error_tempat_lahir; ?></span>
						</div>
					</div> <br>

					<!-- Tanggal Lahir Siswa -->
					<div class="form-group row">
						<label for="tanggal_lahir" class="col-sm-3 col-form-label">Tanggal Lahir</label>
						<div class="col-sm-9">
							<input type="date" name="tanggal_lahir" class="form-control <?php echo ($error_tanggal_lahir !="" ? "is_invalid" : ""); ?>" id="tanggal_lahir" value="<?php echo $tanggal_lahir; ?>"><span class="warning"><?php echo $error_tanggal_lahir; ?></span>
						</div>
					</div> <br>

					<!-- Agama Siswa -->
					<div class="form-group row">
						<label for="agama" class="col-sm-3 col-form-label">Agama</label>
						<div class="col-sm-9">
							<select type="text" name="agama" class="form-control <?php echo ($error_agama !="" ? "is_invalid" : ""); ?>" id="agama" placeholder="Agama" value="<?php echo $agama; ?>">
								<option value=""></option>
								<option value="Islam" <?php if (isset($agama) && $agama == "Islam") echo "selected";?>>Islam</option>
								<option value="Protestan" <?php if (isset($agama) && $agama == "Protestan") echo "selected";?>>Kristen Protestan</option>
								<option value="Katholik" <?php if (isset($agama) && $agama == "Katholik") echo "selected";?>>Kristen Katholik</option>
								<option value="Hindu" <?php if (isset($agama) && $agama == "Hindu") echo "selected";?>>Hindu</option>
								<option value="Buddha" <?php if (isset($agama) && $agama == "Buddha") echo "selected";?>>Buddha</option>
								<option value="Konghucu" <?php if (isset($agama) && $agama == "Konghucu") echo "selected";?>>Kong Hu Cu</option>
								<option value="Lainnya" <?php if (isset($agama) && $agama == "Lainnya") echo "selected";?>>Lainnya</option>	
							</select>
							<span class="warning"><?php echo $error_agama; ?></span>
						</div>
					</div> <br>

					<!-- Hobi Siswa -->
					<div class="form-group row">
						<label for="hobi" class="col-sm-3 col-form-label">Hobi</label>
						<div class="col-sm-9">
							<select type="text" name="hobi" class="form-control <?php echo ($error_hobi !="" ? "is_invalid" : ""); ?>" id="hobi" placeholder="Hobi" value="<?php echo $hobi; ?>">
								<option value=""></option>
								<option value="Olahraga" <?php if (isset($hobi) && $hobi == "Olahraga") echo "selected";?>>Olahraga</option>
								<option value="Kesenian" <?php if (isset($hobi) && $hobi == "Kesenian") echo "selected";?>>Kesenian</option>
								<option value="Membaca" <?php if (isset($hobi) && $hobi == "Membaca") echo "selected";?>>Membaca</option>
								<option value="Menulis" <?php if (isset($hobi) && $hobi == "Menulis") echo "selected";?>>Menulis</option>
								<option value="Travelling" <?php if (isset($hobi) && $hobi == "Travelling") echo "selected";?>>Travelling</option>
								<option value="Lainnya" <?php if (isset($hobi) && $hobi == "Lainnya") echo "selected";?>>Lainnya</option>
							</select>
							<span class="warning"><?php echo $error_hobi; ?></span>
						</div>
					</div> <br>

					<!-- Cita-cita Siswa -->
					<div class="form-group row">
						<label for="citacita" class="col-sm-3 col-form-label">Cita-cita</label>
						<div class="col-sm-9">
							<select type="text" name="citacita" class="form-control <?php echo ($error_citacita !="" ? "is_invalid" : ""); ?>" id="citacita" placeholder="Cita-cita" value="<?php echo $citacita; ?>">
								<option value=""></option>
								<option value="PNS" <?php if (isset($citacita) && $citacita == "PNS") echo "selected";?>>PNS</option>
								<option value="ABRI" <?php if (isset($citacita) && $citacita == "ABRI") echo "selected";?>>ABRI</option>
								<option value="Guru/Dosen" <?php if (isset($citacita) && $citacita == "Guru/Dosen") echo "selected";?>>Guru/Dosen</option>
								<option value="Dokter" <?php if (isset($citacita) && $citacita == "Dokter") echo "selected";?>>Dokter</option>
								<option value="Politikus" <?php if (isset($citacita) && $citacita == "Politikus") echo "selected";?>>Politikus</option>
								<option value="Wirausaha" <?php if (isset($citacita) && $citacita == "Wirausaha") echo "selected";?>>Wirausaha</option>
								<option value="Seniman" <?php if (isset($citacita) && $citacita == "Seniman") echo "selected";?>>Seniman</option>
								<option value="Lainnya" <?php if (isset($citacita) && $citacita == "Lainnya") echo "selected";?>>Lainnya</option>
							</select>
							<span class="warning"><?php echo $error_citacita; ?></span>
						</div>
					</div> <br>

					<!-- Untuk Siswa Berkebutuhan Khusus -->
					<div class="form-group row">
						<label for="berkebutuhan_khusus" class="col-sm-3 col-form-label">Berkebutuhan Khusus</label>
						<div class="col-sm-9">
							<input type="text" name="berkebutuhan_khusus" class="form-control <?php echo ($error_berkebutuhan_khusus !="" ? "is_invalid" : ""); ?>" id="berkebutuhan_khusus" placeholder="Isi Sesuai Keterangan" value="<?php echo $berkebutuhan_khusus; ?>"><small>01=Tidak, 02=Netra (A), 03=Rungu (B), 04=Grahita Ringan (C), 05=Grahita Sedang (C1), 06=Daksa Ringan (D), 07=Daksa Ringan (D1), 08=Laras (E), 09=Wicara (F), 10=Tuna Ganda (G), 11=Hiper Aktif (H), 12=Cerdas Istimewa (i), 13=Bakat Istimewa (J), 14=Kesulitan Belajar (K), 15=Narkoba (N), 16=Indigo (O), 17=Down Sindrome (P), 18=Autis (Q)</small><span class="warning"><?php echo $error_berkebutuhan_khusus; ?></span>
						</div>
					</div> <br>

					<!-- Alamat Jalan Rumah Siswa -->
					<div class="form-group row">
						<label for="alamat" class="col-sm-3 col-form-label">Alamat Jalan</label>
						<div class="col-sm-9">
							<input type="text" name="alamat" class="form-control <?php echo ($error_alamat !="" ? "is_invalid" : ""); ?>" id="alamat" placeholder="Alamat Jalan" value="<?php echo $alamat; ?>"><span class="warning"><?php echo $error_alamat; ?></span>
						</div>
					</div><br>

					<!-- RT -->
					<div class="form-group row">
						<label for="rt" class="col-sm-3 col-form-label">RT</label>
						<div class="col-sm-9">
							<input type="text" name="rt" class="form-control <?php echo ($error_rt !="" ? "is_invalid" : ""); ?>" id="rt" placeholder="RT" value="<?php echo $rt; ?>"><span class="warning"><?php echo $error_rt; ?></span>
						</div>
					</div><br>

					<!-- RW -->
					<div class="form-group row">
						<label for="rw" class="col-sm-3 col-form-label">RW</label>
						<div class="col-sm-9">
							<input type="text" name="rw" class="form-control <?php echo ($error_rw !="" ? "is_invalid" : ""); ?>" id="rw" placeholder="RW" value="<?php echo $rw; ?>"><span class="warning"><?php echo $error_rw; ?></span>
						</div>
					</div><br>

					<!-- Nama Dusun -->
					<div class="form-group row">
						<label for="dusun" class="col-sm-3 col-form-label">Dusun</label>
						<div class="col-sm-9">
							<input type="text" name="dusun" class="form-control <?php echo ($error_dusun !="" ? "is_invalid" : ""); ?>" id="dusun" placeholder="Dusun" value="<?php echo $dusun; ?>"><span class="warning"><?php echo $error_dusun; ?></span>
						</div>
					</div><br>

					<!-- Nama Kelurahan -->
					<div class="form-group row">
						<label for="desa" class="col-sm-3 col-form-label">Desa/Kelurahan</label>
						<div class="col-sm-9">
							<input type="text" name="desa" class="form-control <?php echo ($error_desa !="" ? "is_invalid" : ""); ?>" id="desa" placeholder="Desa/Kelurahan" value="<?php echo $desa; ?>"><span class="warning"><?php echo $error_desa; ?></span>
						</div>
					</div><br>

					<!-- Nama Kecamatan -->
					<div class="form-group row">
						<label for="kecamatan" class="col-sm-3 col-form-label">Kecamatan</label>
						<div class="col-sm-9">
							<input type="text" name="kecamatan" class="form-control <?php echo ($error_kecamatan !="" ? "is_invalid" : ""); ?>" id="kecamatan" placeholder="Kecamatan" value="<?php echo $kecamatan; ?>"><span class="warning"><?php echo $error_kecamatan; ?></span>
						</div>
					</div><br>

					<!-- Nama Kabupaten/Kota -->
					<div class="form-group row">
						<label for="kecamatan" class="col-sm-3 col-form-label">Kabupaten</label>
						<div class="col-sm-9">
							<input type="text" name="kabupaten" class="form-control <?php echo ($error_kabupaten !="" ? "is_invalid" : ""); ?>" id="kabupaten" placeholder="Kabupaten/Kota" value="<?php echo $kabupaten; ?>"><span class="warning"><?php echo $error_kabupaten; ?></span>
						</div>
					</div><br>

					<!-- Kode Pos -->
					<div class="form-group row">
						<label for="kode_pos" class="col-sm-3 col-form-label">Kode Pos</label>
						<div class="col-sm-9">
							<input type="text" name="kode_pos" class="form-control <?php echo ($error_kode_pos !="" ? "is_invalid" : ""); ?>" id="kode_pos" placeholder="Kode Pos" value="<?php echo $kode_pos; ?>"><span class="warning"><?php echo $error_kode_pos; ?></span>
						</div>
					</div><br>

					<!-- Tempat Tinggal Siswa -->
					<div class="form-group row">
						<label for="tempat_tinggal" class="col-sm-3 col-form-label">Tempat Tinggal</label>
						<div class="col-sm-9">
							<select type="text" name="tempat_tinggal" class="form-control <?php echo ($error_tempat_tinggal !="" ? "is_invalid" : ""); ?>" id="tempat_tinggal" placeholder="Tempat Tinggal Sekarang" value="<?php echo $tempat_tinggal; ?>">
								<option value=""></option>
								<option value="Bersama Orang Tua" <?php if (isset($tempat_tinggal) && $tempat_tinggal == "Bersama Orang Tua") echo "selected";?>>Bersama Orang Tua</option>
								<option value="Wali" <?php if (isset($tempat_tinggal) && $tempat_tinggal == "Wali") echo "selected";?>>Wali</option>
								<option value="Kos" <?php if (isset($tempat_tinggal) && $tempat_tinggal == "Kos") echo "selected";?>>Kos</option>
								<option value="Asrama" <?php if (isset($tempat_tinggal) && $tempat_tinggal == "Asrama") echo "selected";?>>Asrama</option>
								<option value="Panti" <?php if (isset($tempat_tinggal) && $tempat_tinggal == "Panti") echo "selected";?>>Panti Asuhan</option>
								<option value="Kediaman Bruce Wayne" <?php if (isset($tempat_tinggal) && $tempat_tinggal == "Kediaman Bruce Wayne") echo "selected";?>>Kediaman Bruce Wayne</option>
								<option value="Lainnya" <?php if (isset($tempat_tinggal) && $tempat_tinggal == "Lainnya") echo "selected";?>>Lainnya</option>
							</select>
							<span class="warning"><?php echo $error_tempat_tinggal; ?></span>
						</div>
					</div><br>

					<!-- Transportasi yang digunakan ke sekolah -->
					<div class="form-group row">
						<label for="transportasi" class="col-sm-3 col-form-label">Transportasi</label>
						<div class="col-sm-9">
							<input type="text" name="transportasi" class="form-control <?php echo ($error_transportasi !="" ? "is_invalid" : ""); ?>" id="transportasi" placeholder="Moda Transportasi yang Anda Gunakan" value="<?php echo $transportasi; ?>"><small>01=Jalan Kaki, 02=Kendaraan Pribadi, 03=Kendaraan Umum/Angkot/Pete-pete, 04=Jemputan Sekolah, 05=Kereta Api, 06=Ojek, 07=Andong/Bendi/Sado/Dokar/Delman/Becak, 08=Perahu Penyeberangan/Rakit/Getek, 09=Lainnya</small><span class="warning"><?php echo $error_transportasi; ?></span>
						</div>
					</div><br>

					<!-- Jarak Siswa dengan sekolah -->
					<div class="form-group row">
						<label for="jarak" class="col-sm-3 col-form-label">Jarak ke Sekolah (KM)</label>
						<div class="col-sm-9">
							<input type="text" name="jarak" class="form-control <?php echo ($error_jarak !="" ? "is_invalid" : ""); ?>" id="jarak" placeholder="Jarak ke Sekolah" value="<?php echo $jarak; ?>"><span class="warning"><?php echo $error_jarak; ?></span>
						</div>
					</div><br>

					<!-- Nomor HP Siswa -->
					<div class="form-group row">
						<label for="hp" class="col-sm-3 col-form-label">No. HP</label>
						<div class="col-sm-9">
							<input type="text" name="hp" class="form-control <?php echo ($error_hp !="" ? "is_invalid" : ""); ?>" id="hp" placeholder="Masukkan Nomor HP" value="<?php echo $hp; ?>"><span class="warning"><?php echo $error_hp; ?></span>
						</div>
					</div><br>

					<!-- Nomor Telepon Rumah Siswa -->
					<div class="form-group row">
						<label for="telp" class="col-sm-3 col-form-label">No. Telepon</label>
						<div class="col-sm-9">
							<input type="text" name="telp" class="form-control <?php echo ($error_telp !="" ? "is_invalid" : ""); ?>" id="telp" placeholder="Masukkan Nomor Telpon Rumah Anda" value="<?php echo $telp; ?>"><small>*Ketikkan 0 jika tidak memiliki telepon rumah</small><span class="warning"><?php echo $error_telp; ?></span>
						</div>
					</div><br>

					<!-- Email Pribadi Siswa -->
					<div class="form-group row">
						<label for="email" class="col-sm-3 col-form-label">E-mail Pribadi</label>
						<div class="col-sm-9">
							<input type="text" name="email" class="form-control <?php echo ($error_email !="" ? "is_invalid" : ""); ?>" id="email" placeholder="Email" value="<?php echo $email; ?>"><span class="warning"><?php echo $error_email; ?></span>
						</div>
					</div><br>

					<h4 class="alert alert-primary text-center mt-1">
							DATA ORANG TUA / WALI MURID
					</h4>
					<!--Ibu-->
					<div class="form-group row">
						<label for="nama_ibu" class="col-sm-3 col-form-label">Nama Lengkap Ibu</label>
						<div class="col-sm-9">
							<input type="text" name="nama_ibu" class="form-control <?php echo ($error_nama_ibu !="" ? "is_invalid" : ""); ?>" id="nama_ibu" placeholder="Nama Lengkap Ibu Anda" value="<?php echo $nama_ibu; ?>"><span class="warning"><?php echo $error_nama_ibu; ?></span>
						</div>
					</div><br>

					<div class="form-group row">
						<label for="pekerjaan_ibu" class="col-sm-3 col-form-label">Pekerjaan Ibu</label>
						<div class="col-sm-9">
							<input type="text" name="pekerjaan_ibu" class="form-control <?php echo ($error_pekerjaan_ibu !="" ? "is_invalid" : ""); ?>" id="pekerjaan_ibu" placeholder="Pekerjaan Ibu Anda" value="<?php echo $pekerjaan_ibu; ?>"><span class="warning"><?php echo $error_pekerjaan_ibu; ?></span>
						</div>
					</div><br>

					<div class="form-group row">
						<label for="pendidikan_ibu" class="col-sm-3 col-form-label">Pendidikan Ibu</label>
						<div class="col-sm-9">
							<input type="text" name="pendidikan_ibu" class="form-control <?php echo ($error_pendidikan_ibu !="" ? "is_invalid" : ""); ?>" id="pendidikan_ibu" placeholder="Pendidikan Ibu Anda" value="<?php echo $pendidikan_ibu; ?>"><small>SD, SMP, SMA, Sarjana, Lainnya</small><span class="warning"><?php echo $error_pendidikan_ibu; ?></span>
						</div>
					</div><br>

					<div class="form-group row">
						<label for="no_ibu" class="col-sm-3 col-form-label">Nomor HP</label>
						<div class="col-sm-9">
							<input type="text" name="no_ibu" class="form-control <?php echo ($error_no_ibu !="" ? "is_invalid" : ""); ?>" id="no_ibu" placeholder="Nomor HP Ibu Anda" value="<?php echo $no_ibu; ?>"><span class="warning"><?php echo $error_no_ibu; ?></span>
						</div>
					</div><br><br>

					<!--Bapak-->
					<div class="form-group row">
						<label for="nama_bapak" class="col-sm-3 col-form-label">Nama Lengkap Bapak</label>
						<div class="col-sm-9">
							<input type="text" name="nama_bapak" class="form-control <?php echo ($error_nama_bapak !="" ? "is_invalid" : ""); ?>" id="nama_bapak" placeholder="Nama Lengkap Bapak Anda" value="<?php echo $nama_bapak; ?>"><span class="warning"><?php echo $error_nama_bapak; ?></span>
						</div>
					</div><br>

					<div class="form-group row">
						<label for="pekerjaan_bapak" class="col-sm-3 col-form-label">Pekerjaan Bapak</label>
						<div class="col-sm-9">
							<input type="text" name="pekerjaan_bapak" class="form-control <?php echo ($error_pekerjaan_bapak !="" ? "is_invalid" : ""); ?>" id="pekerjaan_bapak" placeholder="Pekerjaan Bapak Anda" value="<?php echo $pekerjaan_bapak; ?>"><span class="warning"><?php echo $error_pekerjaan_bapak; ?></span>
						</div>
					</div><br>

					<div class="form-group row">
						<label for="pendidikan_bapak" class="col-sm-3 col-form-label">Pendidikan Bapak</label>
						<div class="col-sm-9">
							<input type="text" name="pendidikan_bapak" class="form-control <?php echo ($error_pendidikan_bapak !="" ? "is_invalid" : ""); ?>" id="pendidikan_bapak" placeholder="Pendidikan Bapak Anda" value="<?php echo $pendidikan_bapak; ?>"><small>SD, SMP, SMA, Sarjana, Lainnya</small><span class="warning"><?php echo $error_pendidikan_bapak; ?></span>
						</div>
					</div><br>

					<div class="form-group row">
						<label for="no_bapak" class="col-sm-3 col-form-label">Nomor HP</label>
						<div class="col-sm-9">
							<input type="text" name="no_bapak" class="form-control <?php echo ($error_no_bapak !="" ? "is_invalid" : ""); ?>" id="no_bapak" placeholder="Nomor HP Bapak Anda" value="<?php echo $no_bapak; ?>"><span class="warning"><?php echo $error_no_bapak; ?></span>
						</div>
					</div><br><br>

					<!--Wali-->
					<div class="form-group row">
						<label for="nama_wali" class="col-sm-3 col-form-label">Nama Lengkap Wali</label>
						<div class="col-sm-9">
							<input type="text" name="nama_wali" class="form-control <?php echo ($error_nama_wali !="" ? "is_invalid" : ""); ?>" id="nama_wali" placeholder="Nama Lengkap Wali Anda" value="<?php echo $nama_wali; ?>"><span class="warning"><?php echo $error_nama_wali; ?></span>
						</div>
					</div><br>

					<div class="form-group row">
						<label for="pekerjaan_wali" class="col-sm-3 col-form-label">Pekerjaan Wali</label>
						<div class="col-sm-9">
							<input type="text" name="pekerjaan_wali" class="form-control <?php echo ($error_pekerjaan_wali !="" ? "is_invalid" : ""); ?>" id="pekerjaan_wali" placeholder="Pekerjaan Wali Anda" value="<?php echo $pekerjaan_wali; ?>"><span class="warning"><?php echo $error_pekerjaan_wali; ?></span>
						</div>
					</div><br>

					<div class="form-group row">
						<label for="pendidikan_wali" class="col-sm-3 col-form-label">Pendidikan Wali</label>
						<div class="col-sm-9">
							<input type="text" name="pendidikan_wali" class="form-control <?php echo ($error_pendidikan_wali !="" ? "is_invalid" : ""); ?>" id="pendidikan_wali" placeholder="Pendidikan Wali Anda" value="<?php echo $pendidikan_wali; ?>"><small>SD, SMP, SMA, Sarjana, Lainnya</small><span class="warning"><?php echo $error_pendidikan_wali; ?></span>
						</div>
					</div><br>

					<div class="form-group row">
						<label for="no_wali" class="col-sm-3 col-form-label">Nomor HP</label>
						<div class="col-sm-9">
							<input type="text" name="no_wali" class="form-control <?php echo ($error_no_wali !="" ? "is_invalid" : ""); ?>" id="no_wali" placeholder="Nomor HP Wali Anda" value="<?php echo $no_wali; ?>"><span class="warning"><?php echo $error_no_wali; ?></span>
						</div>
					</div><br>

					<h4 class="alert alert-primary text-center mt-1">
							DATA LAINNYA
					</h4> 

					<!-- Radio Button Penerima KPS/PKH/KIP -->
					<div class="form-group row">
						<label for="penerima_kps" class="col-sm-3 col-form-label">Penerima KPS/PKH/KIP</label>
						<div class="col-sm-9">
							<input type="radio" name="penerima_kps" value="Ya">Ya
							<input type="radio" name="penerima_kps" value="Tidak">Tidak
							<span class="warning"><?php echo $error_penerima_kps; ?></span>
						</div>
					</div><br>

					<!-- Nomor KPS/PKH/KIP -->
					<div class="form-group row">
						<label for="no_kps" class="col-sm-3 col-form-label">Nomor KPS/KKS/PKH/KIP</label>
						<div class="col-sm-9">
							<input type="text" name="no_kps" class="form-control <?php echo ($error_no_kps !="" ? "is_invalid" : ""); ?>" id="no_kps" placeholder="Nomor KPS/KKS/PKH/KIP" value="<?php echo $no_kps; ?>"><small>* Apabila Menerima (Ketikkan 0 jika tidak menerima)</small><span class="warning"><?php echo $error_no_kps; ?></span>
						</div>
					</div><br>

					<!-- Kewarganegaraan Siswa -->
					<div class="form-group row">
						<label for="kewarganegaraan" class="col-sm-3 col-form-label">Kewarganegaraan</label>
						<div class="col-sm-9">
							<input type="radio" name="kewarganegaraan" value="WNI">WNI
							<input type="radio" name="kewarganegaraan" value="WNA">WNA
							<span class="warning"><?php echo $error_kewarganegaraan; ?></span>
						</div>
					</div><br>

					<!-- Asal Negara Siswa -->
					<div class="form-group row">
						<label for="nama_negara" class="col-sm-3 col-form-label">Nama Negara</label>
						<div class="col-sm-9">
							<input type="text" name="nama_negara" class="form-control <?php echo ($error_nama_negara !="" ? "is_invalid" : ""); ?>" id="nama_negara" placeholder="Nama Negara" value="<?php echo $nama_negara; ?>"><span class="warning"><?php echo $error_nama_negara; ?></span>
						</div>
					</div><br>

					<!-- Alasan Siswa Mendaftar -->
					<div class="form-group row">
					<label for="alasan" class="col-sm-3 col-form-label">Alasan Mendaftar</label>
					<div class="col-sm-9">
						<textarea name="alasan" class="form-control <?php echo ($error_pesan !="" ? "is-invalid" : ""); ?>"><?php echo $alasan; ?></textarea><span class="warning"><?php echo $error_alasan; ?></span>
					</div>				
				</div><br>

					<!--Tombol Submit atau Reset Data Inputan-->
					<button type="submit" class="btn btn-primary">SIMPAN</button>
					<button type="reset" class="btn btn-danger">RESET</button>
						
				</form>
			</div>
		</div>
	</div>
</div>

</body>
</html>